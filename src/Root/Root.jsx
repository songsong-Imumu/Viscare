...
export default class Root extends React.Component {
  ...
  render() {
    const { Year, Province, Pattern, Province_Array, DrawLine, Cluster } = this.state;
    return (
      <div id="Root">
        ...
        <NMap Year={Year} Pattern={Pattern} DrawLine={DrawLine} callback={this.getProvince}></NMap>
  ...
  getAttribute = (d) => {
    ...
    this.setState({ Year: Year, Province: Province, Pattern: Pattern, DrawLine: DrawLine });
  };
  getParallelYear = (d) => {
    const { Year, Province_Array, Cluster } = d;
    this.setState({ Year: Year, Province_Array: Province_Array, Cluster: Cluster });
  }
  getProvince = (d) => {
    const { Province } = d;
